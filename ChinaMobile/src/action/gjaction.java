...
public class gjaction {
	...
	public String group() {
		List list = is.GWareList("select g.* ,w.name,w.price AS WARE_PRICE,w.photo "
						...
						currpage, pagecount);
		Integer count = is.GWareCount("select count(*) from (select g.* ,w.name,w.price   AS WARE_PRICE,w.photo from ware_group_buying g left join ware w on g.ware_id=w.id where g.state='启用'  and w.state='启用' order by g.id)");
		json.put("data", list);
